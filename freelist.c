    ...
    for (int i = 0; i < NBuffers; i++) {
        ...
        if (BUF_STATE_GET_REFCOUNT(local_buf_state) == 0) {
            ...
            if (timestamp < min) {
                ...
                bufID1 = bufID;
                bufID = i;
            } else if (timestamp < second_smallest && timestamp != min) {
                second_smallest = timestamp;
                bufID1 = i;
            }
        }
...
/*	int min = 0;
	...
	// make another for loop, nbuffer as the iterator, use if statement, condition is >min, you 
	for(i = 0; i < length(NBuffers); i++){
		...
		else if(timestamp > second_smallest){}
			second_smallest = timestamp;
			
	}
	printf("Candidate Buffers: %d",NBuffers);
	printf("Evicted: %d",second_smallest);
*/
	if (StrategyControl->firstFreeBuffer >= 0)
	{
		while (true)
...
//GOESHERHERHERHERHERH
//if (buf->timestamp > min && buf->timestamp < second_smallest)
	// find the second smallest one
//	for(i = 0; i < length(GetBufferDescriptor(ClockSweeptick())); i++){
//		buf = GetBufferDescriptor(i);
//		buf -> timestamp;
//		if(timestamp > min){
//			timestamp = min;
